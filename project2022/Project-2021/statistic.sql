...
SELECT EXTRACT(year FROM m.release_Date) AS year, COUNT (m.id)
FROM "Movie" m
GROUP BY EXTRACT(year FROM m.release_Date)
HAVING EXTRACT(year FROM m.release_Date) is not null
...
SELECT EXTRACT(year FROM m.release_Date) AS year, g.name, COUNT (m.id)
FROM "Movie" m , "Movie_Genres" mg, "Genre" g
WHERE m.id = mg.movie_id AND g.id = mg.genre_id
GROUP BY EXTRACT(year FROM m.release_Date), g.name
HAVING EXTRACT(year FROM m.release_Date) is not null
ORDER BY EXTRACT(year FROM m.release_Date), g.name;
...
WHERE m.id = mc.movie_id AND mc.person_id = 4173
GROUP BY EXTRACT(year FROM m.release_Date)
HAVING EXTRACT(year FROM m.release_Date) is not null
ORDER BY EXTRACT(year FROM m.release_Date);
...
SELECT g.name, CAST(AVG(r.rating) AS DECIMAL(10,2))
FROM "Genre" g, "Movie_Genres" mg, "Ratings" r
